...
contract SeedCouncilProofs is Script {
    struct ProofSeed {
        string label;
        string proposalText;
        string proposalUrl;
        ...
        ProofSeed[3] memory seeds = [
            ProofSeed({
                ...
                proposalText: vm.envOr(
                    "COUNCIL_YES_PROPOSAL_TEXT", string("Approve a 500,000 USDC Q3 community grants program.")
                ),
                proposalUrl: vm.envOr(
                    "COUNCIL_YES_URL", string("https://steward-ashy.vercel.app/proposals/community-grants.html")
                ),
                resolveUrl: vm.envOr("COUNCIL_YES_RESOLVE_URL", false)
            }),
            ProofSeed({
                label: "NO",
                proposalText: vm.envOr(
                    "COUNCIL_NO_PROPOSAL_TEXT", string("Unlock 10 percent of foundation team tokens six months early.")
                ),
                proposalUrl: vm.envOr(
                    "COUNCIL_NO_URL", string("https://steward-ashy.vercel.app/proposals/team-token-unlock.html")
                ),
                resolveUrl: vm.envOr("COUNCIL_NO_RESOLVE_URL", false)
            }),
            ProofSeed({
                label: "ABSTAIN",
                proposalText: vm.envOr(
                    "COUNCIL_ABSTAIN_PROPOSAL_TEXT",
                ...
                resolveUrl: vm.envOr("COUNCIL_ABSTAIN_RESOLVE_URL", false)
            })
        ];
